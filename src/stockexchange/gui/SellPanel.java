...
import stockexchange.entity.offerSell.OfferSellManager;
import stockexchange.entity.offerSell.OfferSellModel;
...
public class SellPanel extends JPanel implements ActionListener {
    ...
    private final OfferSellManager dataManager;

    public SellPanel(String mode, SessionManager sessionManager) {
	dataList = new ArrayList();
	dataManager = new OfferSellManager(mode, sessionManager);
    ...
    private void init() {
	JLabel label = new JLabel(" Предложения продажи (SELL)");
	label.setOpaque(true);
	label.setBackground(Color.LIGHT_GRAY);

	dataTable.setBackground(new Color(242, 219, 219));
	OfferSellModel dm = new OfferSellModel(dataList);
	TableRowSorter<OfferSellModel> sorter = new TableRowSorter<>(dm);
